...
function removeFirstOccurrences(str, value) {
  ...
  const strStart = str.slice(0, index);
  const strFinish = str.slice(index + value.length);
  return index !== -1 ? strStart.concat(strFinish) : str;
}
...
function isPalindrome(str) {
  const value1 = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  if (value1 === '') return false;
  const revers = value1.split('').reverse().join('');
  return value1 === revers;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    result += index === -1 ? str[i] : alphabetROT[index];
  }
  return result;
}
